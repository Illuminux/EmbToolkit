...
QT_MKSPEC_DIR	:= $(QT_SRC_DIR)/mkspecs/qws/linux-$(STRICT_GNU_TARGET)-g++
...
define embtk_beforeinstall_qt
	...
	then \
		if ! [ -f dl/qt-everywhere-opensource-src-4.8.6.patch ]; \
		then \
			cp -f packages/misc/qt/qt-everywhere-opensource-src-4.8.6.patch dl/; \
		fi; \
		if ! [ -f "$(QT_SRC_DIR)/.patched" ]; \
		then \
			cd $(QT_BUILD_DIR); \
			patch -p1 < ../../../dl/qt-everywhere-opensource-src-4.8.6.patch; \
			touch $(QT_BUILD_DIR)/.patched; \
		fi; \
	fi; \
